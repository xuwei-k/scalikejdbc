  ...
  describe("StatementExecutor") {
    it("should work with JSR-310 APIs") {
      DB.autoCommit { s =>
        ...
        val alertTime = driverClassName match {
          case "org.hsqldb.jdbc.JDBCDriver" | "com.mysql.jdbc.Driver" =>
        ...
        val localCreatedAt = LocalDateTime.now.plusNanos(222000)
        val createdAt = ZonedDateTime.now.plusNanos(333000)
        val updatedAt = Instant.now.plusNanos(444000)
        val deletedAt = OffsetDateTime.now.plusNanos(555000)
