...
object Binders {
  ...
  val javaTimeLocalDateTime: Binders[java.time.LocalDateTime] = utilDate.xmap(nullThrough(_.toLocalDateTime), v => java.util.Date.from(v.atZone(GlobalSettings.zoneId).toInstant))
