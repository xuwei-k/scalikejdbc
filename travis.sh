...
if [[ ${TRAVIS_SCALA_VERSION} = "scripted-test" ]]; then
  ...
  cd $TRAVIS_BUILD_DIR &&
  sbt -XX:MaxPermSize=2G -XX:ReservedCodeCacheSize=2G -XX:+CMSClassUnloadingEnabled '++ 2.11.2' root211/publishLocal '++ 2.10.4' publishLocal mapper-generator/scripted
else
