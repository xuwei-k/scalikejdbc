...
if [[ ${TRAVIS_SCALA_VERSION} = "scripted-test" ]]; then
  ...
  cd $TRAVIS_BUILD_DIR &&
  git add . --all &&
  sbt '++ 2.11.5' root211/cleanupTestDatabase root211/publishLocal '++ 2.10.4' publishLocal checkScalariform mapper-generator/scripted
else
  git add . --all &&
  sbt ++${TRAVIS_SCALA_VERSION} "project root211" cleanupTestDatabase test:compile checkScalariform testSequential
fi
