...
private class StreamingContext(subscriber: Subscriber[_], database: StreamingDB, val bufferNext: Boolean) extends Subscription with LogSupport {
  /**
   ...
   */
  @volatile private[scalikejdbc] var sync = 0

  private[scalikejdbc] var currentSession: DBSession = null
   ...
   */
  private[scalikejdbc] var deferredError: Throwable = null

  /**
   * The state for a suspended streaming action. Must only be set from a synchronous action
   * context.
   */
  private[scalikejdbc] var streamState: AnyRef = null

  /** The streaming action which may need to be continued with the suspended state */
  private[scalikejdbc] var streamingAction: StreamingAction[_, _ <: WithExtractor] = null
  ...
  def restartStreaming(): Unit = {
    ...
    if (s ne null) {
      ...
      database.scheduleSynchronousStreaming(a, this)(s.asInstanceOf[a.State])
    } else {
